...
이 절차는 커밋 전에 변경 파일과 직접 연관된 파일의 주석을 보강할 때 따른다.
...
3. 커밋 대상 파일마다 직접 연관된 파일을 필요한 만큼 확인한다.
   - 호출자와 피호출자
   - 상속/구현 관계
   - 같은 기능의 설정, 애노테이션, 테스트
   - 변경 메서드의 기존 사용 예제
4. 사용자가 커밋을 요청한 경우에도, 주석 변경 후 실제 커밋 전에는 변경 범위가 요청과 맞는지 다시 확인한다.
 ...
 * scheduler/async worker 처럼 request 가 없을 수 있는 흐름에서는 {@link #getRequestOrNull()}을 사용한다.</p>
 */
