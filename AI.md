...
- AI Agent의 새 세션이 시작되면 반드시 `.AI/공통정책/사용자역할과-작업영역-확인-정책.md`를 먼저 읽고,
역할과 작업 영역이 확인될 때까지 모든 개발 작업을 중단한다.
역할과 작업 영역은 가능하면 선택 UI로 확인해야 하며,
작업 영역 선택지는 현재 저장소의 실제 패키지 구조를 조사해서 생성한다.
`com.{companyName}._{domainName}` 같은 추상 예시 문구를 사용자 선택지로 그대로 보여주지 않는다.
