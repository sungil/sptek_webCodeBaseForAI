...
public class ViewLoginRedirectHelper {
     ...
     */
    public String getRedirectUrlAfterLogging(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (isRedirectAllowed(finalRefererPath) && isSafeContextRelativeRedirectUrl(redirectParam)) {
     ...
     */
    public boolean hasOkRedirectUrlSpringOwn(HttpServletRequest request, HttpServletResponse response) {
        ...
        return (!savedRequestRedirectPath.isEmpty() && isRedirectAllowed(finalSavedRequestRedirectPath));
    }

    /**
     * Spring Security가 저장한 redirect URL을 조회한다.
     */
    public String getRedirectUrlSpringOwn(HttpServletRequest request, HttpServletResponse response) {
     ...
     */
    public void removeRedirectUrlSpringOwn(HttpServletRequest request, HttpServletResponse response) {
        //request.getSession().removeAttribute("SPRING_SECURITY_SAVED_REQUEST"); // todo : 어떤게 더 좋은 방법일까?
        requestCache.removeRequest(request, response);
    }

    /**
     * 같은 application 안의 context-relative URL인지 확인한다.
     */
    public static boolean isSafeContextRelativeRedirectUrl(String redirectUrl) {
        if (!StringUtils.hasText(redirectUrl)) return false;
        if (!redirectUrl.startsWith("/")) return false;
        if (redirectUrl.startsWith("//")) return false;
        if (redirectUrl.contains("\\")) return false;
        if (redirectUrl.contains("\r") || redirectUrl.contains("\n")) return false;

        try {
            URI uri = new URI(redirectUrl);
            return uri.getScheme() == null && uri.getHost() == null && uri.getRawPath() != null;
        } catch (URISyntaxException e) {
            return false;
        }
    }

    private boolean isRedirectAllowed(String path) {
        return StringUtils.hasText(path)
                && properties.getNotRedirectUrls().stream().noneMatch(url -> url.equals(path));
    }
}
