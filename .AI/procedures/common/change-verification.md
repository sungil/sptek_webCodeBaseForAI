...
- 신규 동작이나 버그 수정에는 가능하면 `src/test/java`에 회귀 테스트를 추가한다.
...
- 공개 API 요청/응답 방식 변경: 컨트롤러, DTO validation, 공통 응답 래핑, 보안 matcher, HTTP 예제, 관련 문서를 함께 검토한다.
- 디렉터리나 파일의 위치 또는 이름 변경: 변경 전 경로와 파일명을 참조하는 문서, 코드, 설정, 스크립트를 검색하고 변경된 경로와 이름에 맞게 함께 수정한다.
