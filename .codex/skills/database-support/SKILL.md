...
- DB 작업 안전 기준은 `_AI/_작업정책/DB작업-공통-정책.md`를 따른다.
- H2 File DB를 다룰 때는 `_AI/_작업정책/H2-DB-작업-정책.md`를 함께 확인한다.
- 상세 조회 절차는 `_AI/_작업수행절차/읽기전용-DB조회-절차.md`를 따른다.
- JDBC 기반 조회는 `_AI/_ai-generated/_scripts/db/query-jdbc.ps1`를 우선 사용한다.
- 반복 SQL은 `_AI/_ai-generated/_snippets/sql` 아래에 저장하거나 재사용한다.
